...
//Case insensitive check for the strings "hello" or "bye"
...
void checkForHelloOrBye(char* input){
    ...
    while (i < 5){
        char inputLowerCase = tolower(input[i]);
        helloCheck[i] = inputLowerCase;
        if (i < 3){
            byeCheck[i] = inputLowerCase;
        }
        i++;
    }

    if (strcmp(helloCheck, "hello") == 0){
        helloFlag = 1;
    }
    else{
        helloFlag = 0;
